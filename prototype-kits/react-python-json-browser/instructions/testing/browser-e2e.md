...
- Opener controls and submit actions are different: click an auxiliary opener such as `control.open-create-<entity>`, then scope the submit/save click to the visible form and submit action anchor. The opener is not the scheme action; the submit/save control is.
...
- Treat create/edit/delete/search/filter/tab/navigation as async UI state transitions. Wait for a positive visible result before count or absence assertions. Before full-list count assertions, explicitly reset search/filter state and wait for a known row/card to be visible.
