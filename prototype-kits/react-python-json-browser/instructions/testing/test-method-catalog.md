...
- For repeated table/list rows, assertions for field values must be row-scoped and field/cell-scoped. Avoid `getByText()` for short or numeric values such as stock quantity, price fragments, status codes, or category labels when they may appear in more than one cell. Prefer stable field display anchors or `row.locator('td').nth(<known-column-index>)` within the already-located row.
...
- Cover explicit search dimensions from the requirement. If a requirement says users can search by `name`, `email`, or `phone`, the UI must make all of those user-visible search dimensions reachable: either one generic text search that searches across all named fields, or explicit controls for each named field. The compact e2e spec should exercise each explicitly named search dimension at least once when there are only a few; otherwise document which dimensions are representative and why.
- After a mutation changes any visible field used later for row lookup or search, use the current updated value for subsequent assertions. This includes title/name as well as email, phone, status, category, date, or any requirement-named search dimension. If a test edits phone and later searches by phone, it must search by the edited phone, not the original phone.
...
- When a product Clear/Reset control exists, the e2e must click that control from a non-empty search/filter state. Do not pre-clear the input before clicking the product control; otherwise the test can hide a stale-state implementation bug. If the reset is supposed to return to an unfiltered/full list, prove it with two runtime-owned rows/cards that differ on the active search/filter dimension, not with the same row that was already visible under the filter.
