...
- The durable outputs of a phase are the JSON files explicitly required by that phase prompt. Stdout summaries and internal agent todo lists are not pipeline artifacts.
- Do not rely on optional OpenCode todo tools (`todowrite` / `todoread`) as part of the pipeline contract. If the runtime uses them internally, they must not replace the required JSON reports.
- Do not call optional todo tools merely to mirror `file_plan.json`; the approved file plan, `implementation_report.json`, `change_manifest.json`, and `repair_report.json` are the structured progress and change records.
...
- If an OpenCode shell/bash diagnostic needs a timeout, use the runtime's exact schema: prefer omitting tool-level timeout entirely, and never pass it as a float or quoted string such as `30000.0`.
