...
- For repeated table rows, keep assertions scoped to the row and then to the intended cell or field. Do not assert short, numeric, or repeated values with `row.getByText('10')`, `row.getByText('0')`, or page-wide text selectors; they can match several cells in the same row. Prefer a stable field/display anchor such as `field.<entity>-stock-quantity-display` when the UI provides it, or a scoped column assertion such as `await expect(row.locator('td').nth(4)).toContainText(currentStock);`.
- Do not use `.first()`, `.last()`, or `.nth()` merely to hide duplicate matches from ambiguous locators. Use `.nth()` only when it represents a known table column inside a specific row/card. If Playwright strict mode reports duplicate text matches inside a row, repair the selector by scoping to the intended cell/field rather than changing product code or accepting any match.
